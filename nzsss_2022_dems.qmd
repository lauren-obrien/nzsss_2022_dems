...
leaflet() |>
  ...
  addRasterImage(mw, opacity = 0.5, colors = pal, group = 'elevation')|> 
...
ggplot(mw_df) +
  ...
  #annotate(geom = 'rect', xmin =, ymin, xmax, ymax)
...
Most high-resolution DEMs have a certain amount of local surface roughness. Some of that is random noise, some of it is related to vegetation patterns (like grassed areas) or landscape alterations (like tillage marks), and some is left over from standard production steps like building removal. The variability often only ranges across a few centimeters, but it can cause exaggerated and messy results when calculating slope and other derived parameters.
...
Unfortunately `whitebox` can't directly consume in-memory R objects like the one above, so we must write the DEM to an on-disk file that it can access:
...
ggplot() +
  ...
  geom_raster(data = mw_fps_df, aes(x = x , y = y, fill = maungawhau_hr_FPS),
              alpha = 0.5) +
  scale_fill_scico(palette = 'batlow') +
  theme_void() +
  theme(axis.title = element_blank(),
...
ggplot(hss) +
  ...
  theme(text = element_text(family = 'Ebrima',
                            size = 18)) +
  # bug: scalebar and facets interact to screw with factor ordering
  #ggspatial::annotation_scale(data = scale_params,
  #                            aes(
